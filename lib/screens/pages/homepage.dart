...
import 'package:recipe_app/screens/pages/recipecarousal.dart';
import 'package:recipe_app/screens/pages/seeall.dart';
import 'package:recipe_app/utils/app_theme.dart';
import 'package:recipe_app/widgets/CategoryBar.dart';
import 'package:recipe_app/widgets/barsearch.dart';
...
class _HomePageState extends State<HomePage> {
  List<Recipe> _vegRecipe=[];
  List<Recipe> _nonvegRecipe=[];
  
  List<Recipe> _recipes = [
    Recipe(
        "Taco",
        "Taco for breakfast",
        "assets/images/let_us_taco_bout_it.jpg",
        250,
        35,
        6,
        "Sujan",
    ...
    Recipe("Savory", "Tasty Savory", "assets/images/savory.jpg", 250, 35, 6,
        "Sujan", "Non-veg", [
      "3 – 6 skinless, boneless chicken thighs",
      "1 pound (450g) green beans, trimmed",
      "3 tablespoons butter, divided or (ghee for paleo diet)",
  ...
  List<Recipe> vegrecipe(){
    for(var i=0; i<_recipes.length;i++){
      if(_recipes[i].category!="Non-veg"){
        _vegRecipe.add(_recipes[i]);
      }
    }
    return _vegRecipe;
  ...
  List<Recipe> nonvegrecipe(){
    for(var i=0; i<_recipes.length;i++){
      if(_recipes[i].category=="Non-veg"){
        _nonvegRecipe.add(_recipes[i]);
      }
    }
    return _nonvegRecipe;
              ...
              child: Column(children: [
                ...
                Container(
                  ...
                  child: ListView(
                    ...
                    children: [
                      CategoryBar(onTap:()=> Navigator.push(context,
                          MaterialPageRoute(builder: (_) =>
                              SeeAll(name: "See All", recipeList: _recipes))),
                          icon: Icon(Icons.explore),
                          name: "See All"),
                      SizedBox(width: 5.0,),
                      CategoryBar(onTap:()=> Navigator.pushNamed(context, "favourite"), icon: Icon(Icons.favorite_border_outlined), name: "Favourite"),
                      SizedBox(width: 5.0,),
                      CategoryBar(onTap: ()=>Navigator.push(context,
                          MaterialPageRoute(builder: (_) =>
                              SeeAll(name: "Non- Veg", recipeList: _nonvegRecipe))),
                          icon: Icon(Icons.explore), name: "Non-Veg"),
                      SizedBox(width: 5.0,),
                      CategoryBar(onTap: ()=>Navigator.push(context,
                          MaterialPageRoute(builder: (_) =>
                              SeeAll(name: "Veg", recipeList: _vegRecipe))), icon: Icon(Icons.explore), name: "Veg"),
                ...
                RecipeCarousal(recipes: _recipes),
                SizedBox(
                  height: 16,
                ),
                Align(
  ...
  Widget buildPopular(Recipe recipe) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context, MaterialPageRoute(builder: (_) => Detail(recipe: recipe)));
      },
      child: Container(
